...
/**
 * バイト単位で管理される文字列クラス。 <p> HSP の文字列が MS932 なのに対して Java は UCS-2、また HSP
 * の文字列が可変に対して、Java の文字列は不変など、 多くの場面で Java
 * の文字列をそのまま使用することが出来ないので、代わりにこのクラスを使用する。 </p>
 *
 * @author Yuki
...
public class ByteString implements Serializable {
     ...
     * @param uniqueBuffer このオブジェクトが bytes とは別のバッファを使用するかどうか。 false
    ...
    /**
     * 指定されたバイト配列を使用するオブジェクトを構築する。 <p>実際の文字列の長さが length より短いときは、実際の長さが優先される。
     * </p>
     *
     * @param bytes MS932 で文字列を保持するバイトの配列。
     * @param offset 文字列を開始するオフセット。
     ...
     *
     * @param str 初期かもと文字列。
     * @param uniqueBuffer このオブジェクトが uniqueBuffer とは別のバッファを使用するかどうか。 false
     * の方が高速ですが、文字列を変更予定のときは true を指定してください。
     */
